...
train_dataset = TitanicDataset(r"D:\MyPython\DeepLearning\DeepLearning_Demo\titanic\train1.csv")
validation_dataset = TitanicDataset(r"D:\MyPython\DeepLearning\DeepLearning_Demo\titanic\validation.csv")
...
# 使用tqdm包装DataLoader
    progress_bar = tqdm(DataLoader(train_dataset, batch_size=256, shuffle=True), 
    ...
    for features, labels in progress_bar:
        ...
        progress_bar.set_postfix({
            'Loss': f'{loss.item():.4f}',
            'Acc': f'{torch.sum(((outputs >= 0.5) == labels)).item()/len(labels):.3f}'
        })
    ...
    print(f'Epoch {epoch + 1}/{epochs}, Loss: {total_loss/step:.4f}, '
          f'Training Accuracy: {correct / len(train_dataset):.4f}')

    for features, labels in DataLoader(train_dataset, batch_size=256, shuffle=True):
        print(f"[train loop] batch features.shape: {features.shape}")
        step += 1
        features = features.to("cuda")
        labels = labels.to("cuda")
        optimizer.zero_grad()
